...
#include "robot_arm_msgs/action/robotarm_task.hpp"
...
public:
    ...
    : Node("task_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<robot_arm_msgs::action::RobotarmTask>(
        this, "task_server", 
...
private:
  rclcpp_action::Server<robot_arm_msgs::action::RobotarmTask>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse goalCallback(
      ...
      std::shared_ptr<const robot_arm_msgs::action::RobotarmTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<robot_arm_msgs::action::RobotarmTask>> goal_handle)
  {
    (void)goal_handle;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<robot_arm_msgs::action::RobotarmTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal...");
    auto result = std::make_shared<robot_arm_msgs::action::RobotarmTask::Result>();
    ...
    if (goal->task_number == 0) {
      ...
      arm_target_pose_ = "pre_pick";
      gripper_target_pose_ = "open"; 
      RCLCPP_INFO(get_logger(), "Task 5: Sweeping to Pre-Pick (Ready) Pose");
    }
    else {
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<robot_arm_remote::TaskServer>(options);
