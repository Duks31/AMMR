...
class BasicCameraNode(Node):
    def __init__(self):
        ...
        # Publish to the exact topic you already have open in Foxglove
        self.image_pub = self.create_publisher(
            CompressedImage, "/cika/perception/image_raw/compressed", 10
        )

        # ─── 1. BARE-BONES PIPELINE ────────────────────────────
        ...
        self.device = dai.Device(pipeline, dai.UsbSpeed.HIGH)
        self.q_rgb = self.device.getOutputQueue(name="rgb", maxSize=1, blocking=False)

        # Poll the camera at 10Hz
        self.timer = self.create_timer(0.1, self.publish_frame)
        self.get_logger().info("Basic Node Running. Check Foxglove!")
