#!/usr/bin/env python3
"""
Cika Demo Script Node (Robust Watchdog Version)
-----------------------------------------------
Publishes a continuous /cmd_vel sequence to simulate autonomous
waste detection. Uses a background executor to keep WiFi TCP
hardware watchdogs happy during manual MoveIt pauses.
"""
...
class CikaDemoNode(Node):
    def __init__(self):
        ...
        # State variables for continuous publishing
        self.target_linear = 0.0
        self.target_angular = 0.0
        
        # This timer publishes the target velocity at 20Hz continuously.
        # This prevents the ESP32 WiFi TCP connection from timing out.
        self.timer = self.create_timer(0.05, self.publish_loop)
        self.get_logger().info('Cika continuous publisher ready.')
...
def main(args=None):
    ...
    # Spin the ROS 2 node in a background daemon thread
    # This keeps the publisher timer running and processing DDS discovery
    ...
    try:
        ...
        # Emergency stop override before shutdown
        node.target_linear = 0.0
        node.target_angular = 0.0
        time.sleep(0.1) # Give the background thread a moment to publish the zero
        node.destroy_node()
        rclpy.shutdown()
        sys.exit(0)
